SUBROUTINE hamiltonian(NumG, key, ksi, eta, omega, params, npts, x, wts, dE, S, H, H1, H2, L, M1)
	...
	DOUBLE COMPLEX, INTENT(OUT) :: H(NumG,NumG), H1(NumG,NumG), H2(NumG,NumG), L(NumG,NumG), M1(NumG,NumG)
	...
	DOUBLE COMPLEX ::  b(NumG,NumG), &
			   ...
			   Vmatrix(NumG,NumG), Vmatrix1(NumG,NumG)
	...
	H1(:,:) = SPREAD( -0.5 * ksi, 1, NumG ) * S + &
		  SPREAD( X0 + 0.5 * omega, 1, NumG ) * M1 + &
		  SPREAD( X1, 1, NumG ) * M2 + &
		  SPREAD( X2, 1, NumG ) * M3

	H2(:,:) = SPREAD( X0, 1, NumG ) * M1 + &
	    	  SPREAD( X1, 1, NumG ) * M2 + &
	    	  SPREAD( X2, 1, NumG ) * M3 
	...
	SELECT CASE( key )
		CASE(0)
			...
			Vmatrix1 = SPREAD( X0, 1, NumG ) * M1 + SPREAD( X1, 1, NumG ) * M2 + &
				   SPREAD( X2, 1, NumG ) * M3
		...
		CASE(1)
			DO i = 1, NumG
				DO j = i, NumG
					...
					Vmatrix1(i,j) = 0.5 * ( right_boundary - left_boundary ) * &
						        SUM( wts * &
							     CONJG( wave_packet(npts,new_x,ksi(i),eta(i),omega(i)) ) * new_x * &
							     ( potential_map(npts,new_x,params) + dE ) * &
							     wave_packet(npts,new_x,ksi(j),eta(j),omega(j)) )
					Vmatrix1(j,i) = CONJG( Vmatrix1(i,j) )
