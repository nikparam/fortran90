...
!	WRITE(*,*)
	...
	DO i = 1, NEQ
		DO j = i, NEQ
			left_boundary = MIN( DBLE(ksi(i))/omega(i) - 5 / DSQRT(omega(i)), &
					     DBLE(ksi(j))/omega(j) - 5 / DSQRT(omega(j))   )
					
			right_boundary = MAX( DBLE(ksi(i))/omega(i) + 5 / DSQRT(omega(i)), &
					      DBLE(ksi(j))/omega(j) + 5 / DSQRT(omega(j))   )

!			WRITE(*,*) i, j, left_boundary, right_boundary
