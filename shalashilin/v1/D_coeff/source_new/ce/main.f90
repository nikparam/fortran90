PROGRAM Dcoeff
	...
	ALLOCATE(S(NumG,NumG), H(NumG,NumG), H1(NumG,NumG), H2(NumG,NumG), &
		 M1(NumG,NumG), L(NumG,NumG), R(NumG,NumG), STAT=astatus)
...
!	CALL hamiltonian(NumG, key, ksi, eta, omega, params, npts, x, wts, dE, S, H, H1, H2, L, M1)
!	N = DOT_PRODUCT( Y, MATMUL( S, Y ) )
!	Econst = DOT_PRODUCT( Y, MATMUL( H, Y ) ) / N
	...
	DO 45 WHILE ( TOUT .LE. MAXT )
		...
		CALL overlap(NumG, ksi, eta, omega, S)
		CALL hamiltonian(NumG, key, ksi, eta, omega, params, npts, x, wts, dE, S, H, H1, H2, L, M1)
		CALL coeff_matrix( NumG, m, params, omega, ksi, Y, Econst, dE, S, H, H1, H2, M1, R )
